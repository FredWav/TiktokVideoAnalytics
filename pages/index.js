  ...
  const renderAiAnalysis = (analysis) => {
    ...
    return (
      <ul className="ai-list">
        ...
        <li><strong>Type de contenu :</strong> {analysis.contentType}</li>
        <li><strong>Facteurs viraux :</strong> {analysis.viralFactors?.join(', ')}</li>
        <li><strong>Points faibles :</strong> {analysis.weakPoints?.join(', ')}</li>
      </ul>
    );
  };
    ...
    <>
        ...
        {activeTab === "patterns" && (
          <div className="patterns-section">
            <div className="patterns-header">
              ...
              <div className="niche-selector">{niches.map(niche => <button key={niche.value} className={`niche-btn ${selectedNiche === niche.value ? 'active' : ''}`} onClick={() => setSelectedNiche(niche.value)}><span className="niche-icon">{niche.icon}</span><span className="niche-label">{niche.label}</span></button>)}</div>
            </div>
            {loadingPatterns ? <div className="loading-patterns"><div className="spinner"></div><p>Analyse des patterns...</p></div> : patterns && (
              <div className="patterns-content">
                {patterns.analysisCount === 0 ? <div className="no-data-card"><p>Aucune donnée disponible pour cette niche.</p><p>Analysez des vidéos de cette catégorie pour voir les patterns!</p></div> : <>
                  {patterns.aggregatedStats && <div className="aggregated-stats"><div className="agg-stat"><div className="agg-value">{patterns.analysisCount}</div><div className="agg-label">Vidéos analysées</div></div><div className="agg-stat"><div className="agg-value">{formatNumber(patterns.aggregatedStats.totalViews)}</div><div className="agg-label">Vues totales</div></div><div className="agg-stat"><div className="agg-value">{patterns.aggregatedStats.avgEngagement?.toFixed(1) || 0}%</div><div className="agg-label">Engagement moyen</div></div></div>}
                  {patterns.recentAnalyses && patterns.recentAnalyses.length > 0 && <div className="recent-analyses"><h3>📊 Analyses récentes</h3><div className="analyses-list">{patterns.recentAnalyses.map((analysis, i) => <div key={i} className="analysis-row"><span className="analysis-user">@{analysis.username}</span><span className="analysis-views">{formatNumber(analysis.views)} vues</span><span className={`analysis-engagement ${getEngagementColor(analysis.engagement)}`}>{analysis.engagement?.toFixed(1) || 0}% engagement</span><span className="analysis-time">{new Date(analysis.timestamp).toLocaleDateString('fr-FR')}</span></div>)}</div></div>}
                </>}
              </div>
            )}
          </div>
      ...
      <style jsx global>{`
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .nav-tab { background: transparent; border: 1px solid rgba(255, 255, 255, 0.2); color: white; padding: 0.5rem 1.5rem; border-radius: 0.5rem; cursor: pointer; transition: all 0.3s; }
        ...
        .performance-badge.excellent { border-color: #4ade80; background: linear-gradient(135deg, rgba(74, 222, 128, 0.2), rgba(74, 222, 128, 0.05)); }
        .performance-badge.good { border-color: #60a5fa; background: linear-gradient(135deg, rgba(96, 165, 250, 0.2), rgba(96, 165, 250, 0.05)); }
        .performance-badge.average { border-color: #fbbf24; background: linear-gradient(135deg, rgba(251, 191, 36, 0.2), rgba(251, 191, 36, 0.05)); }
        .performance-badge.low { border-color: #f87171; background: linear-gradient(135deg, rgba(248, 113, 113, 0.2), rgba(248, 113, 113, 0.05)); }
        ...
        .stat-card { background: rgba(255, 255, 255, 0.1); backdrop-filter: blur(10px); border-radius: 0.75rem; padding: 1.5rem; text-align: center; transition: all 0.3s; }
        .stat-card:hover { transform: translateY(-5px); box-shadow: 0 10px 30px rgba(0, 0, 0, 0.3); }
        .stat-icon { font-size: 1.5rem; margin-bottom: 0.5rem; }
        .stat-number { font-size: 1.75rem; font-weight: bold; margin-bottom: 0.5rem; }
        .stat-number.pink { color: #f472b6; } .stat-number.red { color: #f87171; } .stat-number.blue { color: #60a5fa; } .stat-number.green { color: #4ade80; } .stat-number.yellow { color: #fbbf24; }
        .stat-label { font-size: 0.875rem; opacity: 0.75; }
        .stat-rate { font-size: 0.75rem; opacity: 0.6; margin-top: 0.25rem; }
        ...
        .agg-stat { background: rgba(255, 255, 255, 0.1); backdrop-filter: blur(10px); border-radius: 0.75rem; padding: 1.5rem; text-align: center; }
        .agg-value { font-size: 2rem; font-weight: bold; color: #f472b6; margin-bottom: 0.5rem; }
        .agg-label { font-size: 0.875rem; opacity: 0.7; }
        .recent-analyses, .no-data-card { background: rgba(255, 255, 255, 0.1); backdrop-filter: blur(10px); border-radius: 1rem; padding: 2rem; }
        ...
        @media (max-width: 768px) { .hero-title { font-size: 2rem; } .input-container { flex-direction: column; } .analyze-btn { width: 100%; } .nav-content { flex-direction: column; gap: 1rem; } .stats-grid { grid-template-columns: repeat(2, 1fr); } .analysis-row { grid-template-columns: 1fr; gap: 0.5rem; text-align: center; } .analysis-time { text-align: center; } }
      `}</style>
    </>
  );
}
