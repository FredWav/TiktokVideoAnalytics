...
export async function scrapeTikTokVideo(videoUrl) {
  ...
  try {
    ...
    if (sigiMatch) {
      ...
      if (extracted) {
        stats = extracted;
        console.log("Stats extracted from SIGI:", stats);
      }
    }
  } catch (e) {
    console.log("SIGI parsing failed:", e.message);
  }
  ...
  if (stats.views === 0) {
    ...
    for (const pattern of viewPatterns) {
      ...
      if (match && match[1]) {
        ...
        console.log("Found views with pattern:", pattern, "=", stats.views);
        break;
      }
    }
  ...
  if (stats.views === 0) {
    console.log("Searching in all scripts...");
    ...
    for (const scriptTag of scripts) {
      ...
      if (script.includes('playCount') || script.includes('diggCount')) {
        console.log("Found script with stats keywords");
        ...
        for (const jsonStr of jsonMatches) {
          try {
            ...
            if (obj.playCount || obj.stats?.playCount) {
              ...
              console.log("Extracted from JSON object:", stats);
              break;
            }
          } catch (e) {
  ...
  if (stats.views === 0) {
    console.log("Warning: Could not extract views, TikTok may have changed structure");
  }
