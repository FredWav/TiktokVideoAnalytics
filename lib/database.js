...
export async function saveVideoAnalysis(data) {
  try {
    if (USE_VERCEL_KV) {
      ...
      // Remplacer console.log par console.warn pour se conformer aux règles ESLint
      console.warn('Analyse sauvegardée dans Vercel KV:', key);
    } else {
      ...
      console.warn('Analyse sauvegardée localement');
    }
...
async function detectPatterns(newData) {
  try {
    ...
    console.warn('Patterns détectés:', patterns);
    return patterns;
  } catch (error) {
