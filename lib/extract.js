...
export function extractFromHtml(html) {
  ...
  try {
    ...
    if (sigiScript) {
      ...
      if (itemModule) {
        ...
        if (itemId) {
          ...
          console.log("Extraction SIGI_STATE réussie, vues:", result.counts.views);
    ...
    console.log("Erreur parsing SIGI_STATE:", e.message);
  }
  ...
  try {
    ...
    if (ldJsonScript) {
      console.log("JSON-LD trouvé, parsing...");
      ...
      console.log("Extraction JSON-LD, vues:", result.counts.views);
    }
  } catch(e) {
  ...
  if (!result.counts.views) {
    console.log("Tentative extraction regex bruteforce...");
    ...
    if (result.counts.views > 0) {
      console.log("Vues trouvées par regex:", result.counts.views);
  ...
  if (!result.counts.views) {
    console.log("Tentative extraction par sélecteurs CSS...");
    ...
    for (const selector of viewSelectors) {
      ...
      if (element.length) {
        ...
        if (parsed > 0) {
          ...
          console.log(`Vues trouvées via sélecteur ${selector}:`, parsed);
          break;
        }
      }
    }
  ...
  // ========== LOG FINAL ==========
  console.log("Résultat final extraction:", {
